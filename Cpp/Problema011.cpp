...
int main()
{
    ...
    for(int i=0; i<n; ++i){
        for(int j=0; j<n; ++j){
            M[i][j] = stoi(NumberString.substr(pos, 2));
            pos += 3;
        }
    }
    ...
    // Suma de los diagonal izquierdo
    for(int i=l; i<n; ++i){
        for(int j=0; j<=n-l; ++j){
            P = 1;
            for(int k=0; k<l; ++k)
    ...
    cout << "El máximo producto de " << n << " valores es " << maxP << endl;
